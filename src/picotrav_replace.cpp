...
get_new_order(const variable_order vo, net_t& net_0, net_t& net_1){
  std::vector<unsigned> new_ordering;
  switch (vo) {
  ...
  case variable_order::FANIN_DF: {
    get_new_order(variable_order::DF, net_0, net_1);
    new_ordering = fanin_variable_order(net_0);
    break;
  }

  case variable_order::FANIN_DF_LEVEL: {
    get_new_order(variable_order::DF_LEVEL, net_0, net_1);
    new_ordering = fanin_variable_order(net_0);
    break;
  }
...
run_picotrav(int argc, char** argv)
{
  ...
  return run<Adapter>("Picotrav replace", varcount, [&](Adapter& adapter) {
    ...
    std::cout << json::field("construction") << json::array_open << json::endl;
